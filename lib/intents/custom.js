    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            if (address.addressLine1 === null && address.stateOrRegion === null) {
                ...
                response = await getCollectionScheduleResponse(7, address.addressLine1);
    ...
    try {
        ...
        if(conciseCollectionEvents.length === 0) {
            ...
            conciseCollectionEvents.forEach( (event,index) => {
                ...
                eventsText = eventsText + ` ${prefix} is ${helpers.getRelativeCalendarDay(event.date) == "Today" ? '' : 'on '}${helpers.getRelativeCalendarDay(event.date) == "Tomorrow" ? '' : 'on '}${helpers.getRelativeCalendarDay(event.date)} and includes ${helpers.convertArrayToSpokenList(event.services)}.`;
                events++;
            });
            
            const relativeCalendarDay = helpers.getRelativeCalendarDay(event.date);
            eventsText = eventsText + ` ${prefix} is ${relativeCalendarDay === "Today" || relativeCalendarDay === "Tomorrow" ? '' : 'on '}${relativeCalendarDay} and includes ${helpers.convertArrayToSpokenList(event.services)}.`;
            responseMessage = helpers.normalizeResponseText(responseMessage);
            console.log(responseMessage);
            return responseMessage;
        }      
